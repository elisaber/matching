...
#Assumed they are already installed
library(matchingR) #Package that matches up agents in stable pairs
library(ineq) #Package that allows to display inequality (currently not used)
library(rmarkdown) #Package that allows to export R code to Word
library(ggplot2) #Package for rendering graphs
library(dplyr) #Package to allow quick filtering
```

### Assumptions to be set by researcher
Sets network size, gender distribution, preference structure, experiment iterations and random distribution variables
```{r, message = FALSE, warning = FALSE}
#Network size
nmen <- 50
nwomen <- 50
...
repetitions <- 199
#Variables for random distribution
mn <- 1000
sd <- 200
```
...
### Define Function that generates preferences
