...
requiredPackages = c("matchingR", "dplyr", "ggplot2", "rmarkdown", "ineq","reshape")
...
repetitions <- 200 #how many times the experiment is repeated with new set of random preferences
...
y=c()
id=1:nrow(dfWomenFO)
for (i in 1:ncol(dfWomenFO))
{
  x=cbind(x,density(dfWomenFO[,i])$x)
  y=cbind(y,density(dfWomenFO[,i])$y)
}
x=melt(x)
names(x)[3]="x"
y=melt(y)
...
y=c()
id=1:nrow(dfMenFO)
for (i in 1:ncol(dfMenFO))
{
  x=cbind(x,density(dfMenFO[,i])$x)
  y=cbind(y,density(dfMenFO[,i])$y)
}
x=melt(x)
names(x)[3]="xm"
y=melt(y)
...
ggplot(data = W, aes(x=x,y=y,group=Var1)) + 
  geom_line(show.legend=FALSE)+
  geom_line(data=M,aes(x=xm,y=ym,group=Var1_m))  +
  #geom_area(aes(fill = "#FE18AC"))+
  xlab("Matching Quality per Woman") + ylab("Frequency") + #Add axis
  labs(title = paste("Match Utility per Agent"), caption = paste(repetitions, " iterations"), subtitle = (paste("Set size = ", setSize," || men/women (in %) = ", nmen/setSize*100, ":", nwomen/setSize*100, "\nPreference structure individual/collective/homolog (in %) = ", round(wI*100), "/", round(wC * 100), "/", round(wH * 100), sep = ""))) + #add title
  theme(plot.title = element_text(face="bold", size=16, hjust=0))+coord_flip() #format title
 ```

```{r, message = FALSE, warning = FALSE, dpi=300}
dfMenFO$id=1:nrow(dfMenFO)
dfPlot=melt(dfMenFO, id.vars="id")
ggplot(data = dfPlot, aes(x=value, fill=variable)) + 
  geom_density(alpha=0.1,show.legend=FALSE, colour=NA) +
  scale_fill_manual(values = rep("#2595FF", each=repetitions+1)) +
  xlab("Matching Quality per Man") + ylab("Frequency") + #Add axis
  labs(title = paste("Match Utility per Agent"), caption = paste(repetitions, " iterations"), subtitle = (paste("Set size = ", setSize," || men/women (in %) = ", nmen/setSize*100, ":", nwomen/setSize*100, "\nPreference structure individual/collective/homolog (in %) = ", round(wI*100), "/", round(wC * 100), "/", round(wH * 100), sep = ""))) + #add title
  theme(plot.title = element_text(face="bold", size=16, hjust=0)) #format title
...
ggplot(data = dfPlot, aes(x=value, fill=variable)) + 
  geom_density(alpha=0.1,show.legend=FALSE, colour=NA) +
  scale_fill_manual(values = rep("#FE18AC", each=repetitions+1)) +
  xlab("Matching Quality per Woman") + ylab("Frequency") + #Add axis
  labs(title = paste("Match Utility per Agent"), caption = paste(repetitions, " iterations"), subtitle = (paste("Set size = ", setSize," || men/women (in %) = ", nmen/setSize*100, ":", nwomen/setSize*100, "\nPreference structure individual/collective/homolog (in %) = ", round(wI*100), "/", round(wC * 100), "/", round(wH * 100), sep = ""))) + #add title
  theme(plot.title = element_text(face="bold", size=16, hjust=0)) #format title
```
...
ggplot(data = DF, mapping = aes(x= sex, y = avg, fill = sex)) + 
  ...
  labs(title = paste("Average Utility by Gender"), caption = paste(repetitions, " iterations"), subtitle = (paste("Set size = ", setSize," || men/women (in %) = ", nmen/setSize*100, ":", nwomen/setSize*100, "\nPreference structure individual/collective/homolog (in %) = ", round(wI*100), "/", round(wC * 100), "/", round(wH * 100), sep = ""))) + #add title
  theme(plot.title = element_text(face="bold", size=16, hjust=0)) + #format title
