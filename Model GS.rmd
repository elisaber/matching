...
plotMen <- ggplot(data = dfPlot, aes(x=value, fill=variable)) + 
  ...
  labs(caption = paste(repetitions, " iterations")) + #add title
  #theme(plot.title = element_text(face="bold", size=16, hjust=0)) + #format title
  theme_economist_white() +
  coord_flip() +
  ...
  ylim(low=1, high=0) +
  xlim(low=4, high=11)+
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))#use this if using utility
  #xlim(low=0, high=1)
...
#plotMen
#plotMen
p=plot_grid(plotMen,plotWomen)

title <- ggdraw() + draw_label("Matching Satisfaction", fontface='bold')
...
p1=plot_grid(sub,p,ncol=1,rel_heights=c(0.1, 1))
